...
verilog_tokens = {
    ...
    'module': [
        (r'parameter\s+(?:(signed|integer|realtime|real|time)\s+)?(\[[^]]+\])?',
         'parameter_start', 'parameters'),
        (r'(input|inout|output)\s+(?:(reg|supply0|supply1|tri|triand|trior|tri0|tri1|wire|wand|wor|logic)\s+)?(?:(signed)\s+)?(\[[^]]+\])?',
    ...
    'parameters': [
        (r'\s*parameter\s+(?:(signed|integer|realtime|real|time)\s+)?(\[[^]]+\])?',
         'parameter_start'),
        (r'\s*(\w+)[^),;]+', 'param_item'),
        (r',', None),
        (r'[);]', None, '#pop'),
    ...
    'module_port': [
        (r'\s*(input|inout|output)\s+(?:(reg|supply0|supply1|tri|triand|trior|tri0|tri1|wire|wand|wor|logic)\s+)?(signed)?\s*(\[[^]]+\])?',
         'module_port_start'),
        (r'\s*(\w+)\s*,?', 'port_param'),
        (r'[);]', None, '#pop'),
        (r'//#\s*{{(.*)}}\n', 'section_meta'),
