...
vhdl_tokens = {
    ...
    'port_param_type': [
        (r'\s*(in|out|inout|buffer)\s+(\w+)\s*\(', 'port_array_param_type',
         'array_range'),
...
class VhdlParameter():
  ...
  '''
    ...
    def __repr__(self):
        return "VhdlParameter('{}', '{}', '{}')".format(
            self.name, self.mode, self.data_type)
  ...
  '''
    ...
    for pos, action, groups in lex.run(text):
        if action == 'metacomment':
            realigned = re.sub(r'^#+', lambda m: ' ' * len(m.group(0)),
                               groups[0])
        ...
        if action == 'section_meta':
            ...
            if kind == 'function':
                vobj = VhdlFunction(name, cur_package, parameters, groups[0],
                                    metacomments)
            else:
        ...
        elif action in ('array_type', 'file_type', 'access_type',
                        'record_type', 'range_type', 'enum_type',
                        'incomplete_type'):
            ...
            vobj = VhdlConstant(groups[0], cur_package, groups[1],
                                metacomments)
            objects.append(vobj)
            kind = None
            name = None
...
class VhdlExtractor():
  ...
  '''
    ...
    '''
        ...
        subtypes = {
            o.name: o.base_type
            for o in objects if isinstance(o, VhdlSubtype)
        }
